...
/* Versao deste pacote de arquivos. O index.html carrega a MESMA string no meta
...
function atualizarStatus() {
  ...
  const selo = $('#statusApi');
  if (selo && selo.parentElement) selo.parentElement.title = `Radar Institucional ${VERSAO_APP}`;
}
...
function avisarPastaMisturada() {
  ...
  const meta = document.querySelector('meta[name="radar-versao"]');
  const versaoHTML = meta ? meta.getAttribute('content') : null;
  const versaoDivergente = versaoHTML && versaoHTML !== VERSAO_APP;

  if (versaoDivergente) {
    FALHAS_BOOT.push(`index.html e da versao ${versaoHTML} e o app.js e da ${VERSAO_APP}`);
  }
  ...
  d.innerHTML = `<span class="aviso-i">▲</span><div>
    <b>${versaoDivergente
      ? 'O navegador está usando um arquivo antigo.'
      : 'Esta pasta está com arquivos de versões diferentes.'}</b><br>
    ${versaoDivergente
      ? `O <code>index.html</code> desta pasta é da versão <b>${esc(versaoHTML)}</b>, mas o
         <code>js/app.js</code> que rodou é da <b>${esc(VERSAO_APP)}</b>. Você está vendo o
         comportamento da versão antiga — correções recentes não aparecem.`
